...
static esp_err_t save_position_to_nvs(motor_handle_t handle);
static esp_err_t load_position_from_nvs(motor_handle_t handle);
...
esp_err_t motor_init(motor_handle_t *motor_handle) {
    ...
    if (load_position_from_nvs(handle) == ESP_OK) {
    ...
    handle->current_state = MOTOR_STATE_IDLE;
        ...
        switch (handle->current_state) {
            ...
            case MOTOR_STATE_HOMING:
                ...
                if (current_time_ms - move_start_time_ms > CONFIG_PID_FAILURE_TIMEOUT_MS) {
                    ...
                    handle->current_state = MOTOR_STATE_ERROR;
                ...
                if (abs(handle->target_position - handle->current_position) <= CONFIG_POSITION_DEADBAND) {
                    if (handle->current_state == MOTOR_STATE_CALIBRATING) {
                        ESP_LOGI(TAG, "Calibration movement finished. Setting zero point.");
                        handle->current_position = 0;
                        handle->target_position = 0;
                        handle->is_calibrated = true;
                    } else if (handle->current_state == MOTOR_STATE_HOMING) {
                        ESP_LOGI(TAG, "Homing movement finished. Resetting zero point.");
                        handle->current_position = 0;
                        handle->target_position = 0;
                    } else { // MOTOR_STATE_MOVING
                        ESP_LOGI(TAG, "Target reached.");
                    }
                    handle->current_state = MOTOR_STATE_IDLE;
                    save_position_to_nvs(handle);
                }
                break;
...
static void process_command(motor_handle_t handle, const motor_command_t *cmd) {
    switch (cmd->type) {
        case CMD_SET_TARGET:
            ...
            ESP_LOGI(TAG, "CMD: Set target=%" PRId32 ", speed=%.2f", cmd->position, cmd->max_speed_pps);
            ...
            handle->target_position = cmd->position;
            handle->current_state = MOTOR_STATE_MOVING;
            break;
        ...
        case CMD_CALIBRATE:
            ...
            handle->current_state = MOTOR_STATE_CALIBRATING;
            break;
        ...
        case CMD_GO_TO_ZERO:
            ...
            handle->current_state = MOTOR_STATE_HOMING;
            break;
    }
}
...
static void hall_encoder_isr_handler(void* arg) {
    ...
    if (transition == 1) handle->current_position++;
    else if (transition == -1) handle->current_position--;
    
    handle->last_hall_state = current_state;
}

static esp_err_t save_position_to_nvs(motor_handle_t handle) {
    esp_err_t err = nvs_open(NVS_NAMESPACE, NVS_READWRITE, &handle->nvs_handle);
    if (err != ESP_OK) return err;
    err = nvs_set_i32(handle->nvs_handle, NVS_POSITION_KEY, handle->current_position);
    if (err == ESP_OK) err = nvs_commit(handle->nvs_handle);
    nvs_close(handle->nvs_handle);
    return err;
}

static esp_err_t load_position_from_nvs(motor_handle_t handle) {
    esp_err_t err = nvs_open(NVS_NAMESPACE, NVS_READONLY, &handle->nvs_handle);
    if (err != ESP_OK) return err;
    err = nvs_get_i32(handle->nvs_handle, NVS_POSITION_KEY, (int32_t*)&handle->current_position);
    nvs_close(handle->nvs_handle);
    return err;
}
